...
public class InitializedVariables {
    ...
    private Set<CtVariableReference> defined(ControlFlowNode n) {
        ...
        if (n.getStatement() != null) {
            if (n.getStatement() instanceof CtLocalVariable) {
                ...
                if (e instanceof CtVariableAccess) def.add(((CtVariableAccess) e).getVariable());
    ...
    private Set<CtVariableReference> used(ControlFlowNode n) {
        if (n.getStatement() == null) return new HashSet<>();
        //Obtain variables used in this node
        HashSet<CtVariableReference> used = new HashSet<>();
        for (CtVariableAccess a :
                n.getStatement().getElements(new TypeFilter<CtVariableAccess>(CtVariableAccess.class))) {
     ...
     * @return
     */
    private InitFactors initialized(ControlFlowNode n,
                                    HashMap<ControlFlowNode, InitFactors> factors,
                                    boolean includeDefinedInNode) {
        //+ -> Union
        //* -> Intersection
        ...
        for (ControlFlowEdge e : n.getParent().incomingEdgesOf(n)) {
            ...
            depth++;
            InitFactors fp;
            if (factors.containsKey(p)) fp = factors.get(p);
            else fp = initialized(p, factors, true);
            depth--;
            ...
            } else result.defined.retainAll(fp.defined);

            fp.used.removeAll(fp.defined);
            result.used.addAll(fp.used);
        }

        if (result.defined == null) result.defined = defN;
        else result.defined.addAll(defN);
